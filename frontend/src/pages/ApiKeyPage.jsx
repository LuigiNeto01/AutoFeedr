...
export default function ApiKeyPage() {
  ...
  if (loading) {
    return (
      <div className={`h-52 animate-pulse rounded-2xl ${isDarkMode ? "bg-slate-800/70" : "bg-slate-200/70"}`} />
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {success ? (
        <div className={`rounded-xl border px-4 py-3 text-sm ${isDarkMode ? "border-emerald-900/60 bg-emerald-950/40 text-emerald-200" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>
          {success}
        </div>
      ) : null}
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <div className={`rounded-2xl border p-4 shadow-sm ${isDarkMode ? "border-slate-700 bg-slate-900" : "border-slate-200 bg-white"}`}>
          <p className={`text-xs uppercase tracking-[0.18em] ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>
            Status
          </p>
      ...
      <section className={`rounded-2xl border p-4 shadow-sm ${isDarkMode ? "border-slate-700 bg-slate-900" : "border-slate-200 bg-white"}`}>
        <h3 className={`mb-3 text-lg font-semibold ${isDarkMode ? "text-slate-100" : "text-slate-900"}`}>
          {hasKey ? "Atualizar chave OpenAI" : "Salvar chave OpenAI"}
        </h3>
        <form className="space-y-3" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            ...
            <button
              type="button"
              onClick={loadStatus}
              className={`rounded-xl border px-4 py-2 text-sm font-semibold transition ${isDarkMode ? "border-slate-600 text-slate-200 hover:bg-slate-800" : "border-slate-300 text-slate-700 hover:bg-slate-100"}`}
            >
              Recarregar status
            </button>
          </div>
